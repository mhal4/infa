...
int main(){
    ...
    for (int len = 90000; len < 100001; len+=1000){
        for (int i = 0; i<10; i++){
            int sp[len];
            fill_randints(sp, len);
            for (int id = 0 ; id < 6; id++){
                ...
                sort(id, temp, len);
                t_end = get_time();
                arr_to_csv[1] = t_end-t_start;
                
                t_start = get_time();
                sort(id, temp, len);
                t_end = get_time();
                arr_to_csv[2] = t_end-t_start;
                
                reverse(temp, len);
                t_start = get_time();
                sort(id, temp, len);
                t_end = get_time();
                arr_to_csv[3] = t_end-t_start;
                
                write(arr_to_csv,id);
                cout << id << ' ';
                printArray(arr_to_csv,4);
            }
        }
    }
}